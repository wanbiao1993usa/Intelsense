...
#     profile = init_rs_depth()
#     frame = show_stream_depth(profile)
#
#
# -*- coding: utf-8 -*-
# encoding: utf-8print
...
while True:
    ...
    print('camera_coordinate: ',
          camera_coordinate)  # ('camera_coordinate: ', [-0.022640999406576157, -0.03151676058769226, 2.5230000019073486])

    color_point = rs.rs2_transform_point_to_point(depth_to_color_extrin, camera_coordinate)
    color_pixel = rs.rs2_project_point_to_pixel(color_intrin, color_point)
    print('color_point: ',
          color_point)  # ('color_point: ', [-0.022640999406576157, -0.03151676058769226, 2.5230000019073486])
    ...
    for i in range(len(vtx)):
        npy_vtx[i][0] = float(vtx[i][0])
        npy_vtx[i][1] = float(vtx[i][1])
        npy_vtx[i][2] = float(vtx[i][2])

    npy_tex = np.zeros((len(tex), 3), float)
    for i in range(len(tex)):
        npy_tex[i][0] = float(tex[i][0])
        npy_tex[i][1] = float(tex[i][1])
    ...
    cv2.putText(img_color, "Dis:" + str(img_depth[320, 240]) + " m", (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                [255, 0, 255])
